...
HDEC H264Dec_Init()
{
	H264DecData *decData= (H264DecData*)malloc(sizeof(H264DecData));
	if(NULL == decData)
		return NULL;

	memset(decData, 0, sizeof(H264DecData));
	...
	decData->codec = avcodec_find_decoder(AV_CODEC_ID_H264);
	if(!decData->codec)
	{
...
bool H264Dec_Decoder(HDEC hdec, void* h264_buffer, int h264_len, int *width, int *height, void* out_buffer, int * out_len)
{
	...
	//ParseH264Data((unsigned char*)h264_buffer, h264_len);

	H264DecData *decData = (H264DecData*)hdec;

	decData->avpkt.size = h264_len;
	decData->avpkt.data = (unsigned char*)h264_buffer;
